...
function file_loader(folder_path) {
  	...
  	return Promise.all([
		...
		targetSelect.property("value", "Journal");
		dropnaSelect.property("value", "true");
		prizedSelect.property("value", "YES");
		thresholdInput.property("value", 5);
		filter.select("label").html("Journal:");
		filter.select("select").property("name", "Field");
		...
		function getConfigParams() {
			...
			if (keySet.has(filterkey)) {
				...
				filter.select("label").text(newFilterKey + ":");
		...
		[sourceSelect, targetSelect, dropnaSelect, prizedSelect,filter].forEach(control => {
			...
			control.on("change", ()=>{
				...
				if(filter.select("select").property("name")!="Field" && filter.select("select").property("value")!="any"){
					thresholdInput.property("value", 1);
				}
				debouncedDraw();
				updateText();
			});
	...
	const container = d3.select(options.containerTag || "#chart");
	  ...
	  const errorHandler = (err) => {
		...
		errorBox
		  .style("display", "block")
		  .html(`
			<h3>Failed to draw the plot.</h3>
			<p>Error：${err.message}</p>
			<p>You might need to check the threshold value and use a lower one. </p>
		  `);
